...
public class TrustedWebActivityServiceConnectionManager {
    ...
    public boolean notifyNotification(Context appContext, Uri scope, String origin,
        ...
        return execute(appContext, scope, origin, wrapCallback(new ExceptionCallback() {
            ...
            public void onConnected(@Nullable ITrustedWebActivityService service)
                ...
                Bundle result = service.notifyNotificationWithChannel(TrustedWebActivityService
                        .createNotifyNotificationBundle(platformTag, platformId,
                                builder.build(), channelName));

                boolean success = TrustedWebActivityService.getNotifyNotificationSuccess(result);
                // TODO(peconn): success can be false if the client app did not have permissions to
                // show the notification. What do we do here?
            }
        }));
    }
    ...
    public boolean cancelNotification(Context appContext, Uri scope, String origin,
        ...
        return execute(appContext, scope, origin, wrapCallback(new ExceptionCallback() {
            ...
            public void onConnected(@Nullable ITrustedWebActivityService service)
                    throws RemoteException {
                if (service == null) return;
                service.cancelNotification(TrustedWebActivityService
                        .createCancelNotificationBundle(platformTag, platformId));
            }
        }));
    }
    ...
    private boolean execute(final Context appContext, final Uri scope, String origin,
        ...
        if (connection != null) {
            ITrustedWebActivityService service = connection.getService();
            if (service != null) {
                // We have an open connection.
                callback.onConnected(service);
            } else {
                // A connection is being opened, but is not open yet.
