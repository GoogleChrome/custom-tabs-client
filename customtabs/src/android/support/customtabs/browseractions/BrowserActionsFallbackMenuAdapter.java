...
class BrowserActionsFallbackMenuAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (menuItem.getIconId() != 0) {
            ...
            FallbackImageReadTask task = new FallbackImageReadTask(menuItem.getTitle(), mContext.getContentResolver()) {
                @Override
                protected void onBitmapFileReady(Bitmap bitmap) {
                    // ViewHolder has been reused by other item.
                    if (!viewHolder.mText.getText().toString().equals(mText)) return;
                    if (bitmap != null) {
    ...
    private class FallbackImageReadTask extends BrowserServiceImageReadTask {
        String mText;
