...
public class TrustedWebActivityBuilder {
     ...
     *
     * When Trusted Web Activity is on verified origin, the toolbar is hidden, so the color applies
     * only to the status bar. When it's on an unverified origin, the toolbar is shown, and the
     * color applies to both toolbar and status bar.
     */
    @NonNull
    public TrustedWebActivityBuilder setToolbarColor(@ColorInt int color) {
        mToolbarColor = color;
        return this;
    }
    ...
    public void launchActivity(CustomTabsSession session) {
        ...
        if (mToolbarColor != null) {
            intentBuilder.setToolbarColor(mToolbarColor);
        }
    ...
    /**
     * Returns the color set via {@link #setToolbarColor(int)} or null if not set.
     */
    @Nullable
