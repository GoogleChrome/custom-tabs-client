...
public class TrustedWebActivityService extends Service {
    ...
    private final ITrustedWebActivityService.Stub mBinder =
        ...
        @Override
        public Bundle getSmallIconData() {
            checkCaller();

            return TrustedWebActivityService.this.getSmallIconData();
        }
    ...
    private Bundle getSmallIconData() {
        int id = getSmallIconId();
        Bundle bundle = new Bundle();
        bundle.putInt(KEY_SMALL_ICON_ID, id);
        if (id != -1) {
            bundle.putParcelable(
                    KEY_SMALL_ICON_BITMAP, BitmapFactory.decodeResource(getResources(), id));
        }
        return bundle;
    }
