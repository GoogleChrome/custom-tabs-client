...
/**
 * Utils for redirecting share data incoming from other apps into Trusted Web Activities.
 */
public class SharingUtils {
     ...
     * a share intent, i.e. it's action isn't {@link Intent#ACTION_SEND} or
     * {@link Intent#ACTION_SEND_MULTIPLE}.
     */
    @Nullable
    public static ShareData getShareDataFromIntent(Intent intent) {
    ...
    public static ShareTarget parseShareTargetJson(@NonNull String json) throws JSONException {
        ...
        String enctype = object.optString("enctype");
    ...
    private static List<ShareTarget.FileFormField> parseFiles(@Nullable JSONArray filesJson)
        ...
        for (int i = 0; i < filesJson.length(); i++) {
            JSONObject fileObj = filesJson.getJSONObject(i);
            String name = fileObj.getString("name");
            List<String> acceptedTypes = parseAcceptedTypes(fileObj.get("accept"));
